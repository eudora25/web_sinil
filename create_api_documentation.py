...
def create_api_documentation_xlsx():
    # API 데이터 정의
    api_data = [
        {
            "api_name": "사용자 등록 API",
    ...
    for api in api_data:
        summary_data.append([
            ...
            api['description'],
            api['frequency'],
            "POST" if "업로드" in api['api_name'] or "등록" in api['api_name'] or "생성" in api['api_name'] else "GET" if "상태" in api['api_name'] else "POST",
            ...
            "엑셀 파일 처리" if "엑셀" in api['api_name'] else "파일 업로드" if "파일" in api['api_name'] else "사용자 관리" if "사용자" in api['api_name'] else "시스템"
        ])
    ...
    # 파일 저장
    wb.save('API_SWAGGER_DOCUMENTATION.xlsx')
    print("API 문서가 성공적으로 생성되었습니다: API_SWAGGER_DOCUMENTATION.xlsx")

if __name__ == "__main__":
    create_api_documentation_xlsx()
